...
export function SelectionPageClient({ ownerWhatsappNumber }: { ownerWhatsappNumber: string | null }) {
  ...
  return (
    <div className="container-luxe py-16">
      ...
      {/*
        min-w-0 on both grid columns: a CSS grid item's default min-width is
        `auto` (its content's own minimum size), exactly like a flex item —
        without overriding it, a long enough product name or brand can force
        this column wider than the track actually has room for, and every
        min-w-0 fix inside it ends up dividing up that inflated width
        instead of the real available space. This is what let the very
        narrow phone widths still overflow even after the row inside was
        fixed.
      */}
      <div className="grid gap-12 lg:grid-cols-5">
        <div className="min-w-0 lg:col-span-3">
          <div className="divide-y divide-border border-y border-border">
            {/*
              Everything to the right of the image lives in one min-w-0
              flex-1 column, split into two sub-rows (name+remove, then
              quantity+price) instead of one wide row. Packing image, name,
              quantity stepper, price and remove button into a single
              horizontal row left the quantity stepper — three fixed-size
              controls that can't shrink below ~96px — fighting the price
              and remove button for whatever was left, which ran out at the
              narrowest phone widths (~320px) and pushed the row past the
              viewport. Two shorter rows each have their own budget instead.
            */}
            {items.map((item) => (
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <Link
                        ...
                        className="font-serif text-lg hover:text-accent-dark"
                      >
                        {item.name}
                      </Link>
                    ...
                    <button
                      ...
                      className="flex h-9 w-9 shrink-0 cursor-pointer items-center justify-center text-muted-foreground hover:text-destructive"
                    >
                      <X className="h-4 w-4" />
                    </button>
                  </div>
