...
import { SALE_SOURCE_LABELS, PAYMENT_METHOD_LABELS } from "@/types";
  ...
  return (
    <div className="mb-6 flex flex-col gap-3">
      ...
      <div className="flex flex-wrap gap-2">
        <select className={selectClass} value={searchParams.get("source") ?? ""} onChange={(e) => updateParams({ source: e.target.value || null })}>
          <option value="">All Sources</option>
          {Object.entries(SALE_SOURCE_LABELS).map(([v, l]) => <option key={v} value={v}>{l}</option>)}
        </select>

        <select className={selectClass} value={searchParams.get("payment_method") ?? ""} onChange={(e) => updateParams({ payment_method: e.target.value || null })}>
